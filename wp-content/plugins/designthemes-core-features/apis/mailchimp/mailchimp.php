...
function dt_theme_mailchimp_subscribe() {
	...
	if($apiKey != '' && $listId != '') {
		...
		if(dt_theme_mailchimp_check_member_already_registered($data, $apiKey, $listId)) {
			$out = '<span class="error-msg"><b>'.esc_html__('Error:', 'dtthemes-core').'</b> '.esc_html__('You have already subscribed with us !', 'dtthemes-core').'</span>';
		} else {
			$out = dt_theme_mailchimp_register_member($data, $apiKey, $listId);
		}

	} else {
		$out = '<span class="error-msg"><b>'.esc_html__('Error:', 'dtthemes-core').'</b> '.esc_html__('Please make sure valid mailchimp details are provided.', 'dtthemes-core').'</span>';
	}
...
function dt_theme_mailchimp_register_member($data, $apiKey, $listId) {
	...
	if($httpCode == 200) {
		$out = '<span class="success-msg">'.esc_html__('Success! Please check your inbox or spam folder.', 'dtthemes-core').'</span>';
	} else {
