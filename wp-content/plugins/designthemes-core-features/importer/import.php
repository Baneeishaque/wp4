...
class redart_import {
	...
	public $urls	= array(
		'redart'	=> 'http://dummy.wedesignthemes.com/redart/',
		'aquarium'	=> 'http://dummy.wedesignthemes.com/redart/aquarium/',
		'bike'		=> 'http://dummy.wedesignthemes.com/redart/bike/',
		'fitness'	=> 'http://dummy.wedesignthemes.com/redart/fitness/',
		'model'		=> 'http://dummy.wedesignthemes.com/redart/model/',
		'wine'		=> 'http://dummy.wedesignthemes.com/redart/wine/',
	);
	...
	function import_content( $file = 'all.xml.gz' ){
		...
		$xml = REDART_DIR . '/framework/importer/demo/'. $file;
// 		print_r($xml);
	...
	function import_menu_location( $file = 'menu.txt' ){
		...
		$file_path 	= REDART_URI . '/framework/importer/demo/'. $file;
...
// 		print_r($data);
	...
	function import(){
		...
		if( ! class_exists( 'WP_Import' ) ){
			require_once REDART_CORE_PLUGIN . '/importer/wordpress-importer.php';
		}
		...
		if( class_exists( 'WP_Importer' ) && class_exists( 'WP_Import' ) ){
			...
			switch( $_REQUEST['data']['import'] ) {
				...
				case 'all':
					...
					$this->import_options($import_demo.'/options.txt', $this->urls[$import_demo]);
				...
				default:
					// Empty select.import
					$this->error = esc_html__('Please select data to import.','dtthemes-core');	
					break;
			}
			...
			if( $this->error ){
				...
				echo '<strong>'. esc_html__('All done. Have fun!','dtthemes-core') .'</strong>';
			}
		}
		die(0);
	}
...
$dtthemes_import = new redart_import;
