<?php get_header();
	...
	if($GLOBALS['force_enable'] == true)
	  ...
	  $page_layout  = array_key_exists( "layout", $portfolio_settings ) ? $portfolio_settings['layout'] : $GLOBALS['page_layout'];
   ...
   <section id="primary" class="<?php echo esc_attr( $page_layout );?>"><?php
				...
				<article id="post-<?php the_ID();?>" <?php post_class(array('dt-portfolio-single'));?>><?php
                    ...
                    <?php echo !empty( $container_start ) ? $container_start : ''; ?>
                    	...
                    	<div class="dt-portfolio-single-slider-wrapper">
                        	<?php $allowslider = array_key_exists('portfolio-slider', $portfolio_settings) ? '' : '-none'; ?>
                        	<!-- Slider -->
                            <ul class="dt-portfolio-single-slider<?php echo esc_attr( $allowslider );?>">
                            ...
                            <?php if( array_key_exists("items_name",$portfolio_settings) && empty($allowslider)) { ?>
                            		<!-- Pager -->
                            		<div id="bx-pager">
                                    	<?php if( has_post_thumbnail() ){ ?>
                    ...
                    <?php echo !empty( $container_middle ) ? $container_middle : '';?>
                        ...
                        <div class="dt-portfolio-single-details">
                            ...
                            <?php if( array_key_exists('meta_title', $portfolio_settings) ): ?>
                            		<h3><?php esc_html_e('Project Details','dtthemes-core');?></h3>
                                	<ul class="project-details">
                                		<?php foreach( $portfolio_settings['meta_title'] as $key => $title ):
												$iclass = $portfolio_settings['meta_class'][$key];
                                    		...
                                    		<li> <span class="<?php echo esc_html($iclass);?>"></span><p><?php echo redart_wp_kses($value);?></p></li>
                                    	<?php endforeach;?>
                                    </ul>
                            <?php endif;?>
                            </div>
        ...
        <div class="post-nav-container">
        	<div class="post-prev-link"><?php previous_post_link('%link','<i class="fa fa-angle-double-left"> </i>'.esc_html__('Prev Entry','dtthemes-core') );?> </div>
            <div class="post-next-link"><?php next_post_link('%link',esc_html__('Next Entry','dtthemes-core').'<i class="fa fa-angle-double-right"> </i>');?></div>
        </div><!-- **Post Nav - End** --> 
        
        <?php # Related Portfolio
			  $related_post = redart_option('pageoptions','single-portfolio-related');
			 ...
			 if( isset($related_post) && $terms ) :?>
                ...
                <div class="related-portfolios">
                    <h3 class=""><span><?php esc_html_e('Related Projects','dtthemes-core');?></span></h3><?php
					
					$post_class = "portfolio column dt-sc-one-third";
					$post_style = redart_option('pageoptions','single-portfolio-related-style');
        ...
        <?php #Portfolio Comments
			$post_comment = redart_option('pageoptions','single-portfolio-comments');
