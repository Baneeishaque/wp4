<?php 
class Redart_Flickr extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array("classname"=>'widget_flickr', 'description'=>esc_html__('A widget that show last flickr photo streams', 'redart'));
		parent::__construct(false,REDART_NAME.esc_html__(' Flickr Widget','redart'),$widget_options);
	}
	...
	function form($instance) {
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('flickr_id')); ?>"><?php esc_html_e('Flickr ID:','redart');?>
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('How many entries do you want to show:','redart');?>
         ...
         <p><label for="<?php echo esc_attr($this->get_field_id('show'));?>"><?php esc_html_e('What pictures to display','redart'); ?>
         	<select class="widefat" id="<?php echo esc_attr($this->get_field_id('show')); ?>" name="<?php echo esc_attr($this->get_field_name('show')); ?>">
            <?php  $a = array("latest"=>esc_html__("Latest",'redart'),"random"=>esc_html__("Random",'redart'));
         ...
         <p><label for="<?php echo esc_attr($this->get_field_id('size'));?>"><?php esc_html_e('What pictures to display','redart'); ?>
         	<select class="widefat" id="<?php echo esc_attr($this->get_field_id('size')); ?>" name="<?php echo esc_attr($this->get_field_name('size')); ?>">
            <?php  $a = array("s"=>esc_html__("square",'redart'),"t"=>esc_html__("thumbnail",'redart'), "m" => esc_html__("medium",'redart'));
			foreach($a as $key => $value ):
...
<?php
	...
	function widget($args, $instance) {
		...
		echo redart_wp_kses($before_widget);
			...
			src="http'.redart_ssl().'://www.flickr.com/badge_code_v2.gne?count='.$count.'&amp;display='.$show.'&amp;size='.$size.'&amp;layout=x&amp;source=user&amp;user='.$flickr_id.'"></script>'; 
			echo "</div>";
