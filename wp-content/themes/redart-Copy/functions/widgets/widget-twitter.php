<?php
class Redart_Twitter extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array("classname"=>'widget_tweetbox', 'description'=>esc_html__('To show latest twitter tweets', 'redart'));
		parent::__construct(false,REDART_NAME.esc_html__(' Twitter Widget','redart'),$widget_options);
	}

	#2.widget input form in back-end
	function form($instance) {
		$instance = wp_parse_args( (array) $instance,array( 'title' => esc_html__('Latest Tweets','redart'), 'count' => '3', 'username' => '',
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:','redart');?> 
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('consumer_key')); ?>"><?php esc_html_e('Consumer Key:','redart');?> 
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('consumer_secret')); ?>"><?php esc_html_e('Consumer Secret:','redart');?> 
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('access_token')); ?>"><?php esc_html_e('Access Token:','redart');?> 
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('access_token_secret')); ?>"><?php esc_html_e('Access Token Secret:','redart');?> 
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('username')); ?>"><?php esc_html_e('Enter your twitter username:','redart');?>
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('How many entries do you want to show:','redart');?>
        ...
        <p><input type="checkbox"  id="<?php echo esc_attr($this->get_field_id('exclude_replies'));?>" name="<?php echo esc_attr($this->get_field_name('exclude_replies'));?>" 
			<?php checked($exclude_replies); ?> /> <label for="<?php echo esc_attr($this->get_field_id('exclude_replies')); ?>"><?php esc_html_e('Exclude @replies','redart');?></label></p>
            
        <p><input type="checkbox"  id="<?php echo esc_attr($this->get_field_id('time'));?>" name="<?php echo esc_attr($this->get_field_name('time'));?>" 
			<?php checked($time); ?> /> <label for="<?php echo esc_attr($this->get_field_id('time')); ?>"><?php esc_html_e('Show time of tweet','redart');?></label></p>
        ...
        <p><input type="checkbox"  id="<?php echo esc_attr($this->get_field_id('display_avatar'));?>" name="<?php echo esc_attr($this->get_field_name('display_avatar'));?>" 
				<?php checked($display_avatar); ?> /> <label for="<?php echo esc_attr($this->get_field_id('display_avatar')); ?>"><?php esc_html_e('Show user avatar','redart');?></label></p>		
<?php
	}
	...
	function widget($args, $instance) {
		...
		echo redart_wp_kses($before_widget);			

			if ( !empty( $title ) ) { echo redart_wp_kses($before_title . $title . $after_title); };
			...
			if($username && $consumer_key && $consumer_secret && $access_token && $access_token_secret && $count) { 
				...
				if($twitter && is_array($twitter)) {
					foreach( $twitter as $tweet ):
						...
						echo '<li>';
								if( $display_avatar )
									echo '<span class="tweet-thumb"><a href="http'.redart_ssl().'://twitter.com/'.$username.'" title=""><img src="'.$tweet->user->profile_image_url.'" alt="" /></a></span>';
					...
					echo '<li>'.esc_html__('No public Tweets found','redart').'</li>';
				}
				echo "</ul>";
			}
		echo redart_wp_kses($after_widget);
	}
}?>