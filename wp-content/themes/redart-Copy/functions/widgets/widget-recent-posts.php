<?php
class Redart_Recent_Posts extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array('classname'=>'widget_recent_posts', 'description'=>esc_html__('To list out posts', 'redart'));
		parent::__construct(false,REDART_NAME.esc_html__(' Posts','redart'),$widget_options);
	}
	...
	function form($instance) {
	    ...
	    <p><label for="<?php echo esc_attr($this->get_field_id('_post_categories')); ?>">
			<?php esc_html_e('Choose the categories you want to display (multiple selection possible)','redart');?></label>
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('_excerpt')); ?>"><?php esc_html_e('Display title only or title &amp; excerpt','redart');?></label>
           <?php $answers = array('show title only','show title and excerpt');?>
           <select class="widefat" id="<?php echo esc_attr($this->get_field_id('_excerpt')); ?>" name="<?php echo esc_attr($this->get_field_name('_excerpt')); ?>">
        ...
        <p><input type="checkbox"  id="<?php echo esc_attr($this->get_field_id('_enabled_image'));?>" name="<?php echo esc_attr($this->get_field_name('_enabled_image'));?>"
	         <?php checked($_enabled_image); ?> /> <?php esc_html_e("Show Image",'redart');?></p>  

	    <p><label for="<?php echo esc_attr($this->get_field_id('_post_count')); ?>"><?php esc_html_e('No.of posts to show:','redart');?></label>
	...
	function widget($args, $instance) {
		...
		echo "<div class='recent-posts-widget'><ul>";
			 ...
			 while($the_query->have_posts()):
				...
				echo "<li>";
					if(1 == $_enabled_image):
						...
						$image = ( $image != false)? $image[0] : REDART_URI . '/images/dummy-images/post-thumb.jpg';
					...
					if($show_title):
						...
						echo "<div class='entry-title'><h4><a href='".get_permalink()."'>{$title}</a></h4></div>";
						echo "<div class='entry-content'>".redart_excerpt(10)."</div>";
					endif;
			 ...
			 endwhile;
			 	...
			 	echo "<li><h4>".esc_html__('No Posts found','redart')."</h4></li>";
			 endif;
			 wp_reset_postdata();
	 	echo "</ul></div>";
