...
<article id="post-<?php the_ID();?>" <?php post_class($post_classes);?>>
	<?php if( isset($post_meta['show-featured-image'])):?>
			<!-- Featured Image -->
			...
			<div class="entry-details">
				...
				<div class="entry-date">
					...
					<div class="comments <?php echo esc_attr($show_comment_meta);?>"><?php
						comments_popup_link( __('<i class="pe-icon pe-chat"> </i> 0','redart'),
							__('<i class="pe-icon pe-chat"> </i> 1','redart'),
							__('<i class="pe-icon pe-chat"> </i> %','redart'),
							'',
							__('<i class="pe-icon pe-chat"> </i>','redart'));?>
					</div>
				</div><!-- .entry-date -->
				...
				<?php edit_post_link( esc_html__( ' Edit ','redart' ) ); ?>
			</div><!-- .entry-details -->
	<?php else: # Default Post Style ?>
			...
			<div class="entry-details">
				...
				<div class="entry-meta">
					...
					<div class="comments <?php echo esc_attr($show_comment_meta);?>"> / <?php
						comments_popup_link( __('<i class="pe-icon pe-chat"> </i> 0 Comment','redart'),
							__('<i class="pe-icon pe-chat"> </i> 1 Comment','redart'),
							__('<i class="pe-icon pe-chat"> </i> % Comments','redart'), '',
							__('<i class="pe-icon pe-chat"> </i>','redart'));?>
					</div>
					<div class="author <?php echo esc_attr( $show_author_meta );?>">
						/ <i class="pe-icon pe-user"> </i>
						<a href="<?php echo get_author_posts_url(get_the_author_meta('ID'));?>" title="<?php esc_attr_e('View all posts by ', 'redart'); echo get_the_author();?>">
...
<?php # Post Author Information Box
	$post_author_box = redart_option('pageoptions','single-post-authorbox');
	if( isset($post_author_box) ):?>
		<div class="dt-sc-hr"></div>
		<div class="dt-sc-clear"></div>
		<section class="author-info">
        	<h2><?php esc_html_e('About Author','redart');?></h2>
			<div class="thumb">
...
<?php endif;
	...
	# Related Posts
	$related_post = redart_option('pageoptions','single-post-related');
	...
	#Post Comments
	$post_comment = redart_option('pageoptions','single-post-comments');
