<?php
add_filter( 'wp_edit_nav_menu_walker', 'redart_modify_backend_walker' , 10 );
if( !function_exists('redart_modify_backend_walker') ) {
	function redart_modify_backend_walker() {
		return 'RedArt_ModifyBackendMenuWalker';
	}
}
/* ---------------------------------------------------------------------------
 * Admin Menu Walker
 ...
 * --------------------------------------------------------------------------- */
class RedArt_ModifyBackendMenuWalker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			$title = sprintf( esc_html__( '%s (Invalid)', 'redart' ), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( esc_html__('%s (Pending)', 'redart'), $item->title );
		}
		...
		<li id="menu-item-<?php echo esc_attr($item_id); ?>" class="<?php echo esc_attr(implode(' ', $classes )); ?>">
			<dl class="menu-item-bar">
				<dt class="menu-item-handle">
					<span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo redart_wp_kses($submenu_text); ?>><?php esc_html_e( 'sub item', 'redart' ); ?></span></span>
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'redart'); ?>">&#8593;</abbr></a>
							|
							<a href="<?php
								echo wp_nonce_url(
									add_query_arg(
							...
							?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'redart'); ?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item', 'redart'); ?>" href="<?php
							echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
						?>"><?php esc_html_e( 'Edit Menu Item', 'redart' ); ?></a>
					</span>
				</dt>
			</dl>
			...
			<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e( 'Navigation Label', 'redart' ); ?><br />
						<input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
					</label>
				</p>
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e( 'Title Attribute', 'redart' ); ?><br />
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
						...
						<?php esc_html_e( 'Open link in a new window/tab', 'redart' ); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e( 'CSS Classes (optional)', 'redart' ); ?><br />
						<input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
					</label>
				</p>
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e( 'Link Relationship (XFN)', 'redart' ); ?><br />
						<input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
					</label>
				</p>
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
						<?php esc_html_e( 'Description', 'redart' ); ?><br />
						<textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
						<span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'redart'); ?></span>
					</label>
				</p>
                ...
                <p class="field-dt-use-as-megamenu description description-thin">
                    <label for="edit-menu-item-dt-use-as-megamenu-<?php echo esc_attr($item_id); ?>">
                        ...
                        <?php esc_html_e( 'Use As Mega Menu' ,  'redart' ); ?>
                    </label>
                </p>
                ...
                <p class="field-dt-disable-text description description-thin">
                    <label for="edit-menu-item-dt-disable-text-<?php echo esc_attr($item_id); ?>">
                        ...
                        <?php esc_html_e( 'Disable Text' ,  'redart'); ?>
                    </label>
                </p>
                ...
                <p class="field-dt-disable-link description description-thin">
                    <label for="edit-menu-item-dt-disable-link-<?php echo esc_attr($item_id); ?>">
                        ...
                        <?php esc_html_e( 'Disable Link' ,  'redart' ); ?>
                    </label>
                </p>
                ...
                <p class="field-dt-fullwidth description description-thin">
                    <label for="edit-menu-item-dt-fullwidth<?php echo esc_attr($item_id); ?>">
                        ...
                        <?php esc_html_e( 'Enable Full width' ,  'redart' ); ?>
                    </label>
                </p>
                ...
                <p class="field-dt-submenu-column description description-wide">
                    ...
                    <?php esc_html_e( 'Sub Menu Column Layout' ,  'redart' ); ?><br />
                    ...
                    <span class="description"><?php esc_html_e('Select where to align the submenu.',  'redart'); ?></span>
                    </label>
                </p>
                ...
                <p class="field-dt-menuicon description description-wide">
                    ...
                    <?php esc_html_e( 'Menu Icon' ,'redart');?><br/>
                    ...
                    <span class="description"><?php esc_html_e('Please use font awesome icon ',  'redart'); ?></span>
                    </label>
                </p>
                ...
                <p class="field-dt-menu-bg description description-wide">
                    ...
                    <?php esc_html_e( 'Menu Background' ,'redart');?><br/>
                    ...
                    <span class="description"><?php esc_html_e('Please use image url',  'redart'); ?></span>
                    </label>
                </p>
                ...
                <p class="field-dt-submenu-widget description description-wide">
                    ...
                    <?php esc_html_e( 'Display Widget Area' ,  'redart' ); ?><br />
                        <select id="edit-menu-item-dt-submenu-widget-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-dt-submenu-widget" name="dt-submenu-widget[<?php echo esc_attr($item_id); ?>]">
                            <option value=""><?php esc_html_e('Select', 'redart');?></option><?php
                    ...
                    <span class="description"><?php esc_html_e('Select widget area to show.',  'redart'); ?></span>
                    </label>
                </p>
                ...
                <p class="field-dt-widgetarea-column description description-wide">
                        <label for="edit-menu-item-dt-submenu-column-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Fill Widget Area Layout' ,  'redart' ); ?><br />
                        ...
                        <span class="description"><?php esc_html_e('Only works if you choose widget area',  'redart'); ?></span>
                        </label>
                </p>
				...
				<p class="field-move hide-if-no-js description description-wide">
					<label>
						<span><?php esc_html_e( 'Move', 'redart' ); ?></span>
						<a href="#" class="menus-move-up"><?php esc_html_e( 'Up one', 'redart' ); ?></a>
						<a href="#" class="menus-move-down"><?php esc_html_e( 'Down one', 'redart' ); ?></a>
						<a href="#" class="menus-move-left"></a>
						<a href="#" class="menus-move-right"></a>
						<a href="#" class="menus-move-top"><?php esc_html_e( 'To the top', 'redart' ); ?></a>
					</label>
				</p>
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( esc_html__('Original: %s', 'redart'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					...
					); ?>"><?php esc_html_e( 'Remove', 'redart' ); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
						?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'redart'); ?></a>
				</div>
 ...
 * To Save Menu Items
 * --------------------------------------------------------------------------- */
add_action( 'wp_update_nav_menu_item', 'redart_update_menu_item', 10, 3 );
if( !function_exists('redart_update_menu_item') ) {
	function redart_update_menu_item( $menu_id, $menu_item_db, $args ) {
 ...
 * --------------------------------------------------------------------------- */
class RedArt_FrontEndMenuWalker extends Walker {
	...
	function start_lvl( &$output, $depth = 0, $args = array()) {
		...
		if( !($this->mega_active) && ( $depth === 0 ) ){
			$animation = redart_option('layout','menu-hover-style');
			if( !empty( $animation ) ) {
				$animation = "animate ".$animation;
			}
		}
		
		#$boxshadow = ( "true" ==  redart_option('layout','menu-boxshadow') ) ? 'with-box-shadow' : '';
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		if($depth === 0 ) {
			if( $this->mega_active ) {
				...
				$class_names .= ( "true" == redart_option('layout','menu-boxshadow') ) ? ' with-box-shadow ' : '';
				$class_names .= ( "true" == redart_option('layout','menu-title-bg') ) ? ' menu-title-with-bg ' : '';			
				$class_names .= ( "true" == redart_option('layout','menu-links-bg') ) ? ' menu-links-with-bg ' : '';
				$class_names .= ( "true" == redart_option('layout','menu-links-border') ) ? ' menu-links-with-border ' : '';
				$class_names .= ( "true" ==  redart_option('layout','menu-link-arrow') ) ? ' menu-links-with-arrow  '.redart_option('layout','menu-link-arrow-style').' ':'';
