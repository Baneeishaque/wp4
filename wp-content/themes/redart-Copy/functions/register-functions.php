...
if( !function_exists('redart_wp_list_archive') ){
	function redart_wp_list_archive($link_html,$url, $text, $format, $before, $after ) {
		if( $format == 'html' ) {
	...
	add_filter('get_archives_link', 'redart_wp_list_archive', 10, 6);
}
		...
		echo '<div class="breadcrumb">';
			...
			foreach( $breadcrumbs as $bk => $bc ){
				if( strpos( $bc , $separator ) ) {
					...
					echo redart_wp_kses($bc);
				} else {
					if( $i == $count ) $separator = '';
					echo redart_wp_kses($bc . $separator);
				}
				$i++;
			}
		echo '</div>';
 ...
 * break oembeds.
 * --------------------------------------------------------------------------- */
if ( ! function_exists( 'redart_undo_recurrence_oembed_logic' ) ) {
...
add_action( 'init', 'redart_undo_recurrence_oembed_logic' );
