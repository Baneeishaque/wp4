...
if( !function_exists('redart_enqueue_scripts') ) {
	function redart_enqueue_scripts() {
		...
		$data = "var dttheme_urls = {\n";
		$data .= "\t\t theme_base_url:'".esc_js(REDART_URI)."'";
		...
		wp_add_inline_script('ui.totop', $data, 'before');
...
if( !function_exists('redart_enqueue_styles') ) {
	function redart_enqueue_styles() {
		...
		wp_enqueue_style( 'fancybox',		REDART_URI .'/css/jquery.fancybox.css', false, REDART_VERSION, 'all' );
		...
		// skin css
			...
			if($skin != 'custom')	wp_enqueue_style("skin", 	REDART_URI ."/css/skins/$skin/style.css");
		endif;
