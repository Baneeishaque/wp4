...
if(!empty($wtstyle)):
	...
	if( is_page() ):
		...
		redart_show_sidebar('page',$page_id, 'left');
	elseif( is_single() ):
	
		if( is_singular('post') ) {
	
			$id = ($post->ID == 0) ? get_queried_object_id() : $post->ID;
			redart_show_sidebar('post',$id, 'left');
		} elseif( is_singular('dt_portfolios') ) {
	
			$id = ($post->ID == 0) ? get_queried_object_id() : $post->ID;
			redart_show_sidebar('dt_portfolios',$id, 'left');
			...
			$enable = redart_option('woo','show-shop-standard-left-sidebar-for-product-layout');
			if( $enable ):
			...
			redart_show_sidebar('',$id, 'left');
		}
	elseif( class_exists('woocommerce') && is_post_type_archive('product') ):
		redart_show_sidebar('page',get_option('woocommerce_shop_page_id'), 'left');
	elseif( class_exists('woocommerce') && is_product_category() ):
		...
		$enable = redart_option('woo','show-shop-standard-left-sidebar-for-product-category-layout');
		if( $enable ):
			if( is_active_sidebar('shop-everywhere-sidebar-left') ):
				dynamic_sidebar('shop-everywhere-sidebar-left');
			endif;
	...
	elseif( class_exists('woocommerce') && is_product_tag() ):
		...
		$enable = redart_option('woo','show-shop-standard-left-sidebar-for-product-tag-layout');
		if( $enable ):
			if( is_active_sidebar('shop-everywhere-sidebar-left') ):
				dynamic_sidebar('shop-everywhere-sidebar-left');
			endif;
	...
	elseif( is_tax() ):
		...
		if( $tax == 'portfolio_entries' ) {
			...
			$enable = redart_option('pageoptions','show-standard-left-sidebar-for-portfolio-archives');
			if( $enable ):
	...
	elseif( is_archive() || is_search() ):
		...
		$enable = redart_option('pageoptions','show-standard-left-sidebar-for-post-archives');
		if($enable):
			if( is_active_sidebar('standard-sidebar-left') ):
				dynamic_sidebar('standard-sidebar-left');
			endif;
