<?php
$wtstyle = redart_opts_get('wtitle-style', '');
if(!empty($wtstyle)):
	...
	if( is_page() ):
		...
		redart_show_sidebar('page',$page_id, 'right');
	elseif( is_single() ):
	
		if( is_singular('post') ) {
	
			$id = ($post->ID == 0) ? get_queried_object_id() : $post->ID;
			redart_show_sidebar('post',$id, 'right');
		} elseif( is_singular('dt_portfolios') ) {
	
			$id = ($post->ID == 0) ? get_queried_object_id() : $post->ID;
			redart_show_sidebar('dt_portfolios',$id, 'right');
			...
			$enable = redart_option('woo','show-shop-standard-right-sidebar-for-product-layout');
			if( $enable ):
			...
			redart_show_sidebar('',$id, 'right');
		}	
	elseif( class_exists('woocommerce') && is_post_type_archive('product') ):
		redart_show_sidebar('page',get_option('woocommerce_shop_page_id'), 'right');
	elseif( class_exists('woocommerce') && is_product_category() ):
		...
		$enable = redart_option('woo','show-shop-standard-right-sidebar-for-product-category-layout');
		if( $enable ):
			if( is_active_sidebar('shop-everywhere-sidebar-right') ):
				dynamic_sidebar('shop-everywhere-sidebar-right');
			endif;
	...
	elseif( class_exists('woocommerce') && is_product_tag() ):
		...
		$enable = redart_option('woo','show-shop-standard-right-sidebar-for-product-tag-layout');
		if( $enable ):
			if( is_active_sidebar('shop-everywhere-sidebar-right') ):
				dynamic_sidebar('shop-everywhere-sidebar-right');
			endif;
	...
	elseif( is_tax() ):
		...
		if( $tax == 'portfolio_entries' ) {
			...
			$enable = redart_option('pageoptions','show-standard-right-sidebar-for-portfolio-archives');
			if( $enable ):
			...
			$disable = redart_option('pageoptions',$standard);
			if( is_null($disable) ):
				if( is_active_sidebar('standard-sidebar-right') ):
					dynamic_sidebar('standard-sidebar-right');
				endif;
	...
	elseif( is_archive() || is_search() ):
		...
		$enable = redart_option('pageoptions','show-standard-right-sidebar-for-post-archives');
		if($enable):
			if( is_active_sidebar('standard-sidebar-right') ):
				dynamic_sidebar('standard-sidebar-right');
			endif;
