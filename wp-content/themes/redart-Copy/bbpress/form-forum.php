			...
			<fieldset class="bbp-form">
				<legend>
					...
					<?php
						if ( bbp_is_forum_edit() )
							printf( __( 'Now Editing &ldquo;%s&rdquo;', 'redart' ), bbp_get_forum_title() );
						else
							bbp_is_single_forum() ? printf( __( 'Create New Forum in &ldquo;%s&rdquo;', 'redart' ), bbp_get_forum_title() ) : esc_html_e( 'Create New Forum', 'redart' );
					?>
				...
				<?php if ( !bbp_is_forum_edit() && bbp_is_forum_closed() ) : ?>
					...
					<div class="bbp-template-notice">
						<p><?php esc_html_e( 'This forum is closed to new content, however your account still allows you to do so.', 'redart' ); ?></p>
					</div>
				...
				<div>
					...
					<p>
						<label for="bbp_forum_title"><?php printf( esc_html__( 'Forum Name (Maximum Length: %d):', 'redart' ), bbp_get_title_max_length() ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_type"><?php esc_html_e( 'Forum Type:', 'redart' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_status"><?php esc_html_e( 'Status:', 'redart' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_visibility"><?php esc_html_e( 'Visibility:', 'redart' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_parent_id"><?php esc_html_e( 'Parent Forum:', 'redart' ); ?></label><br />

						<?php
							bbp_dropdown( array(
								'select_id' => 'bbp_forum_parent_id',
								'show_none' => esc_html__( '(No Parent)', 'redart' ),
					...
					<div class="bbp-submit-wrapper">
						...
						<button type="submit" tabindex="<?php bbp_tab_index(); ?>" id="bbp_forum_submit" name="bbp_forum_submit" class="button submit"><?php esc_html_e( 'Submit', 'redart' ); ?></button>
...
<?php elseif ( bbp_is_forum_closed() ) : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice">
			<p><?php printf( __( 'The forum &#8216;%s&#8217; is closed to new content.', 'redart' ), bbp_get_forum_title() ); ?></p>
		</div>
	</div>
...
<?php else : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice">
			<p><?php is_user_logged_in() ? esc_html_e( 'You cannot create new forums.', 'redart' ) : esc_html_e( 'You must be logged in to create new forums.', 'redart' ); ?></p>
		</div>
	</div>
