<?php
	#Event Meta Box
	add_action("add_meta_boxes", "redart_event_metabox");
	add_action('save_post','redart_event_meta_save');

	if( !function_exists('redart_event_metabox') ) {
		function redart_event_metabox(){
			add_meta_box("event-template-meta-container", esc_html__('Event Options', 'redart'), "redart_event_settings","tribe_events", "normal", "high");
		}
	}

	if( !function_exists('redart_event_settings') ) {
		function redart_event_settings($args){
			global $post;
			...
			<div id="page-layout" class="custom-box">
				...
				<div class="column five-sixth last">  
					...
					<p class="note"> <?php esc_html_e("You can choose between a left, right or no sidebar layout.", 'redart');?> </p>
				</div>
			</div><!-- Layout End-->
				...
				<div id="left-sidebar-container" class="page-left-sidebar" <?php echo redart_wp_kses($sidebar_left); ?>>
					...
					<div id="page-commom-sidebar" class="sidebar-section custom-box">
						...
						<div class="column five-sixth last"><?php
							...
							<p class="note"> <?php esc_html_e('Yes! to show "Standard Left Sidebar" on this page.', 'redart');?> </p>
						 </div>
				...
				<div id="right-sidebar-container" class="page-right-sidebar" <?php echo redart_wp_kses($sidebar_right); ?>>
					...
					<div id="page-commom-sidebar" class="sidebar-section custom-box page-right-sidebar">
						...
						<div class="column five-sixth last"><?php 
							...
							<p class="note"> <?php esc_html_e('Yes! to show "Standard Right Sidebar" on this page.', 'redart');?> </p>
						 </div>
				...
				<div class="column five-sixth last"> 
					<select name="dttheme-event-post-style" class="dt-chosen-select">
						...
						<?php	$event_styles =  array( 
								  'type1' => esc_html__('Classic','redart'),
								  'type2' => esc_html__('Full Width','redart'),
								  'type3' => esc_html__('Minimal Tab','redart'),
								  'type4' => esc_html__('Clean','redart'),
								  'type5' => esc_html__('Modern','redart')
							  );
							  foreach( $event_styles as $es => $bv ):
								  echo "<option value='{$es}'".selected($selected,$es,false).">{$bv}</option>";
							  endforeach;?>
					</select>
					<p class="note"> <?php esc_html_e("Your event post page show at most selected style.",'redart');?> </p>
				</div>
			</div><!-- Event Style End-->
	...
	if( !function_exists('redart_event_meta_save') ) {
		function redart_event_meta_save($post_id){
