...
if( !function_exists('redart_ajax_option_save') ) {
	function redart_ajax_option_save() {
		...
		if (get_option(REDART_SETTINGS) != $data) {
 ...
 * --------------------------------------------------------------------------- */
add_action('wp_ajax_redart_ajax_importer', 'redart_ajax_importer');
if( !function_exists('redart_ajax_importer') ) {
	function redart_ajax_importer() {
		require_once REDART_CORE_PLUGIN . '/importer/import.php';
	}
}
