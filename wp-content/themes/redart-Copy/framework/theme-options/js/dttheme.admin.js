...
var dtthemeAdmin = {
  ...
  importData:function(){
	...
	jQuery('select.demo', importer).change(function(){
		...
		// demo
		jQuery('.redart-demos', importer).hide();
	...
	jQuery("a.dttheme-import-button").click(function(e){
		if(jQuery(this).hasClass('import-disabled')) {
			...
			jQuery.ajax({
				type:"POST",
				url:ajaxurl,
				data:{action:'redart_ajax_importer', 'data': $data },
  ...
  importOption : function() {
	  jQuery("a#dttheme_import_button").click(function(e){
		  ...
		  if( ans ){
			  var data =  { action : 'redart_backup_and_restore_action', type:'import_options', data : jQuery("#export_data").val()};
			  jQuery('#ajax-feedback').css({display:'block'});
